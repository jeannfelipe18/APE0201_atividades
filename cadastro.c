...
void alterarRegistro(Aluno a[], int totalCadastro, int buscarEmail){
    int opcaoEscolhida;
    if(totalCadastro == 0 ){
        printf("Lista vazia");
        return;
...
int excluirRegistro(Aluno a[], int totalCadastro, int buscarEmail){
    int achou;
    for(int i = 0; i < totalCadastro; i++){
            if(a[i].email == buscarEmail){
                //registro encontrado
...
main(){
    Aluno a[TAM];
    int opcao, totalCadastro = 0, buscarEmail, indice;
